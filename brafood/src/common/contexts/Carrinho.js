...
export default function CarrinhoProvider({ children }) {
  const [carrinho, setCarrinho] = useState([]); //lista inicia vazia
...
export function useCarrinhoContext() {
  ...
  function adicionarProduto(novoProduto) {
    const temOProduto = carrinho.some(item => item.id === novoProduto.id); // Validar pra ver se ja foi marcado
    ...
    novoCarrinho = mudarQuantidade(novoProduto.id, 1); // else
